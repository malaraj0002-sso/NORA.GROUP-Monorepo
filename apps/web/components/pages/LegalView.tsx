    ...
    <>
      ...
      <section className="section-padding bg-background relative overflow-hidden">
        <div className="container-luxury max-w-3xl space-y-8">
          <Reveal>
            <p className="text-base sm:text-lg leading-relaxed text-foreground whitespace-pre-line">
              {t(page.intro, locale)}
            </p>
          </Reveal>
          ...
          {page.sections.map((section, index) => (
            <Reveal key={`${index}-${t(section.heading, locale)}`} delay={40 + index * 20}>
              <article className="rounded-2xl border border-gold-200/60 bg-card p-6 sm:p-8 shadow-sm">
                <h2 className="text-lg sm:text-xl font-bold text-foreground">
                  {t(section.heading, locale)}
                </h2>
                <p className="mt-3 text-sm sm:text-base leading-relaxed text-muted whitespace-pre-line">
